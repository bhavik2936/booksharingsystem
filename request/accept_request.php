...
if(!empty($_SESSION)) 
{
    if (!empty($_SESSION['user_email']) && !empty($data)) 
    {
        try 
        {
            ...
            //     $roww=$stmt->fetch(PDO::FETCH_ASSOC);
            // }
            $stmt_insert = $conn->prepare("UPDATE book_request SET is_borrow_request_approved = 1 WHERE id = '$data->id'");
            ...
            $c=$conn->prepare("SELECT * FROM user_details WHERE user_id = '$i'");
            $c->execute();
            $cre=$c->fetch(PDO::FETCH_ASSOC);
            
            $descr="Your request is accepted for book ".$f['book_title']."You can contact owner with following credentials: ".$cre['email_id'];
            $st="acceptedrequest";
            ...
            $final['message'] = "Request Accepted Sucessfully ! ";
        }
