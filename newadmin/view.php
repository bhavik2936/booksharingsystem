...
if(!isset($_SESSION['name'])){
    ...
    if(isset($_POST['accept'])){
               ...
               // $desc=html_entity_decode($desc1);
                ...
                $query1="select book_title from book_details where book_id=$id";
                $get1=$conn->prepare($query1);
                $get1->execute();
                $row=$get1->fetch(PDO::FETCH_ASSOC);
                $title=$row['book_id'];
                $str=$title."is accepted :".$desc;
                
                $query2="insert into user_notifications (`user_id`,`book_id`,`description`,`notification_type`) values ('$ownerid','$id','$str','approvedbook')";
                //echo $query2;
                $get3=$conn->prepare($query2);
                $get3->execute();
                
                $query3="update book_details set is_approved='1' where book_id=$id";
             ...
             if(isset($_POST['reject'])){
               ...
               // $desc=html_entity_decode($desc1);
                ...
                $query2="insert into user_notifications (`user_id`,`book_id`,`description`,`notification_type`) values ('$ownerid','$id','$desc','rejectedbook')";
                //echo $query2;
                $get3=$conn->prepare($query2);
                $get3->execute();
        ...
        <form action="view.php?id=<?php echo "$id";?>" method="post"> 
          <div class="form-group"> 
            <label for="comment">Remarks:</label> 
              <textarea class="form-control" rows="6" id="editor" name="text"> 
              </textarea> 
             <input type="hidden" name="id" value="<?php echo"$id";?>">
          </div> 
          
           <button class="btn btn-primary" style="margin-bottom:10px;" name="accept">Accept</button>
         </form> 
      </div>
